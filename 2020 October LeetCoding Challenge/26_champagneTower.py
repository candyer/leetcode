...
def champagneTower(poured: int, query_row: int, query_glass: int) -> float:
	'''both query_row and query_glass are 0_indexed'''
	dp = [[0] * i for i in range(1, query_row + 3)]
	dp[0][0] = poured
	for row in range(query_row + 1):
		for col in range(row + 1):
			portion = (dp[row][col] - 1.0) / 2.0
			if portion > 0:
				dp[row + 1][col] += portion
				dp[row + 1][col + 1] += portion
	return min(1, dp[query_row][query_glass])


assert(champagneTower(1, 1, 1) == 0)
assert(champagneTower(2, 1, 1) == 0.5)
assert(champagneTower(100000009, 33, 17) == 1)
assert(champagneTower(11, 3, 2) == 1)
assert(champagneTower(8, 3, 2) == 0.875)
