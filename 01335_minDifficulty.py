...
# difficulties of each day of the d days. The difficulty of a day is the maximum difficulty of a job done in that day.
...
def minDifficulty(jobDifficulty: List[int], d: int) -> int:
	n = len(jobDifficulty)
	if n < d: return -1
	dp = [[float('inf') for i in range(n)] + [0] for day in range(d)]
	for day in range(d):
		for i in range(n):
			maxi = 0
			for j in range(i, day - 1, -1):
				maxi = max(maxi, jobDifficulty[j])
				dp[day][i] = min(dp[day][i], maxi + dp[day - 1][j - 1])
	return dp[-1][-2]


assert(minDifficulty([6, 3, 2, 7, 5, 4], 3) == 15)
...
assert(minDifficulty([11,111,22,222,33,333,44,444], 6) == 843)
assert(minDifficulty([11,111,22,222,33,333,44,444], 8) == 1220)
