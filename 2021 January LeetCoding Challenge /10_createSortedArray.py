...
# For example, if inserting element 3 into nums = [1,2,3,5], the cost of insertion is min(2, 1) (elements 1 
# and 2 are less than 3, element 5 is greater than 3) and nums will become [1,2,3,3,5].
...
def createSortedArray(instructions: List[int]) -> int:
    ...
    for i, num in enumerate(instructions):
        res += min(query(num - 1), i - query(num)) % mod
        update(num)
    return res % mod

assert(createSortedArray([1,5,6,2]) == 1)
assert(createSortedArray([1,2,3,6,5,4]) == 3)
assert(createSortedArray([1,3,3,3,2,4,2,1,2]) == 4)
