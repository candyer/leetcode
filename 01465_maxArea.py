...
# Given a rectangulat h and width w, and two arrays of integers horizontalCuts and 
...
def get_max_block(n: int, array: List[int]) -> int:
	array.extend([0, n])
	array.sort()
	res = 0
...
def maxArea(h: int, w: int, horizontalCuts: List[int], verticalCuts: List[int]) -> int:
	mod = 10**9 + 7
	row_max_block = get_max_block(h, horizontalCuts)
	col_max_block = get_max_block(w, verticalCuts)
	return row_max_block * col_max_block % mod
